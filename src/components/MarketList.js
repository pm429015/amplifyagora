...
const MarketList = ({searchResults}) => {
  ...
  const onNewMarket = (prevQuery, newData) => {
    let updataedQuery = {...prevQuery};
    const updatedMarketList = [
      newData.onCreateMarket,
      ...prevQuery.listMarkets.items
    ]
    updataedQuery.listMarkets.items = updatedMarketList;
    return updataedQuery
  }

  return (
    <Connect
      query={graphqlOperation(listMarkets)}
      subscriptions={graphqlOperation(onCreateMarket)}
                ...
                >
                  <div>
                    <span className="flex">
                      ...
                      <img src="https://icon.now.sh/shopping_cart/f60"
                        alt="Shopping Cart"/>
                    </span>
